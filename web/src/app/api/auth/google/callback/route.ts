...
export async function GET(req: Request) {
  ...
  const live = new URL("/live", appUrl ?? url.origin).toString();
  if (!code || !state || state !== readCookie(req, "unjargon_oauth_state") || !appUrl || !clientId || !clientSecret) return Response.redirect(live);
  ...
  if (!tokens?.access_token) return Response.redirect(live);
  ...
  if (!profile?.sub || !profile.email) return Response.redirect(live);
  ...
  const res = new Response(null, { status: 302, headers: { Location: live } });
